...
![image-20200726135501383](https://i.loli.net/2020/07/26/eLYGVsoPBcyH6mu.png)
...
Map 的键值可以是基本数据类型，也可以是自定义的 message ，但不支持`repeated`关键字。
...
> 建立 TCP 连接 -> 建立 HTTP2 连接 -> 协商配置 -> 发送 Headers （调用的方法）-> 发送参数 -> 返回结果 -> 断开连接
